...
- **DocEntry** — the raw object returned by `getDocsFromFilesystem` (id, slug, data with frontmatter fields like `title`, `navLabel`, `navIcon`, `navHidden`).
...
- Uses `navLabel`/`navIcon` from frontmatter if `label`/`icon` are not provided on the config.
